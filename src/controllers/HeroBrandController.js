...
const createHeroBrand = async (req, res) => {
  try {
    // ✅ FIX 1: Read 'brand' from the request body here.
    ...
    // ✅ FIX 2: Renamed the result to 'newBrand' to avoid variable name conflict.
...
const updateHeroBrand = async (req, res) => {
  try {
    const { id } = req.params;
    // ✅ FIX 3: Read 'brand' from the request body here.
    const { title, description, brand } = req.body;
    const image = req.file;
